...
const createStore = () => {
  return new Vuex.Store({
    ...
    actions: {
      ...
      async find({ commit },  input ) {
        console.log(input)
        let { data } = await this.$axios.get('/api/c1/search/' + input);
        console.log(data);
        commit('getItems', data);
      },

      async add({ commit }, { name, status, time}) {
        console.log(time);
        console.log(name);
        let result = await this.$axios.post('/api/c1/items', { name: name, status: status, time: time });
        let { data } = await this.$axios.get('/api/c1/items')
        commit('getItems', data);
      }
    },
